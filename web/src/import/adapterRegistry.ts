...
export async function detectAdapter(adapters: readonly ImportAdapterLoader[], input: ImportInput): Promise<DetectedAdapter> {
  ...
  let matches = await probeAdapters(candidates, input);
  if (matches.length === 0 && extensionMatches.length > 0) {
    matches = await probeAdapters(adapters.filter((adapter) => !extensionMatches.includes(adapter)), input);
  }
