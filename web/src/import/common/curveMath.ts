...
export function sampleSpline(points: readonly (readonly [number, number])[], progress: number): number {
  return new SplineCurve(points.map(([x, y]) => new Vector2(x, y))).getPoint(progress).y;
}

export function sampleBezierAtX(
  points: readonly [readonly [number, number], readonly [number, number], readonly [number, number], readonly [number, number]],
  targetX: number,
): number {
  const curve = new CubicBezierCurve(...points.map(([x, y]) => new Vector2(x, y)) as [Vector2, Vector2, Vector2, Vector2]);
  let low = 0;
  let high = 1;
  for (let iteration = 0; iteration < 24; iteration++) {
    const middle = (low + high) / 2;
    if (curve.getPoint(middle).x < targetX) low = middle;
    else high = middle;
  }
