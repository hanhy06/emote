...
디스플레이 수는 인스턴스가 선택한 에모트의 노드 수에 따라 결정된다. 그래프에는 측정된 최대 디스플레이 수 220, 436, 888, 1,341, 1,760개를 읽기 쉽게 약 220, 440, 890, 1,340, 1,760개로 표시했다.

MSPT의 `baseline`은 테스트 시작 직전 서버가 보관하던 최근 100틱 평균이다. 테스트 중 MSPT 표본은 Minecraft가 기록한 해당 틱 시간에 `END_SERVER_TICK`에서 실행된 스트레스 테스트 처리 시간을 더해 계산한다. 따라서 그래프의 MSPT는 에모트 코드만의 시간이 아니라 월드 처리 등 기존 서버 부하와 합성 에모트·패킷 부하를 합친 틱 시간에 가깝다.
