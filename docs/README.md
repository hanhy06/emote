...
Account login uses Emote's public Microsoft application ID (`38fe1d33-748d-4f65-926c-82022799df8e`) by default. To use a different registered application, set `EMOTE_MICROSOFT_CLIENT_ID` in the server process environment. Custom applications must support personal Microsoft accounts and device authorization and have access to Xbox/Minecraft services. A Microsoft login alone does not establish that the app can access Minecraft services.
